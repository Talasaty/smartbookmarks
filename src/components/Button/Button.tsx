...
const ButtonStyle = styled.button.attrs({})<Props>`
    ...
    border: ${p => p.borderColor ? `1px solid ${colors[p.borderColor]}` : 'none'};
...
interface Props{
    ...
    borderColor?: 'ligthGray' | 'darkGray' |'white' | 'black' | 'text' | 'blue' | 'blue2' | 'lighthBlue' | 'darkBlue'
    type?: 'tiny' | 'normal' | 'big' | 'free'
}

export const Button: React.SFC<Props> = ({children, backgroundColor, borderColor, type}) => (
    <ButtonStyle backgroundColor={backgroundColor} borderColor={borderColor} type={type}>
        {children}
    </ButtonStyle>
)
